...
from src.models.d_search.algorithms.termination import NullTermination
from src.models.d_search.algorithms.boxes import Boxes
from src.models.d_search.utils.oper import step_size_norm
...
class Continuation:
    ...
    def has_next(self):
        ...
        print('it {}'.format(self.iter), end='\r')
...
class BiDirectionalDsContinuation:
    ...
    def format_results(self, results):
        ...
        for key in ['X', 'F', 'vs', 'as']:
            if key in ['X', 'F']:
                # remove last element to regularize X,F dimensions with vs,as
                compiled_res[key + "_r"] = np.vstack([res['population'][key][:-1, :] for res in results])
