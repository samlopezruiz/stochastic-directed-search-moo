...
from src.models.d_search.core.problem import AutomaticDifferentiationProblem
from src.models.d_search.utils.plot import plot_boxes_3d, plot_points_3d, plot_traces
...
if __name__ == '__main__':
    ...
    # dists = np.apply_along_axis(lambda x: norm(x - ref), 1, pf_moea)
    # ix = np.argmin(dists)
    # x0 = ps_moea[ix, :]

    # x0 = np.array([0.18967827, 0.5681209, 0.52296044, 0.52590637])
    ...
    hv_moea = get_hypervolume(pf_moea, ref=[2., 2., 2.]) if solve_moea else np.nan
    hv_pop = get_hypervolume(results['population']['F'], ref=[2., 2., 2.])
    hv_ref = get_hypervolume(testfun['pf'], ref=[2., 2., 2.])
