...
from src.timeseries.moo.cont.utils.indicators import subset_metrics, pred_corr_metrics
from src.timeseries.moo.cont.utils.results import compile_metrics_from_results, concat_mean_std_cols
...
def get_continuation_method(cont_cfg, problem):
    ...
    predictor = get_predictor(cont_cfg['predictor']['type'],
                              ...
                              max_increment=cont_cfg['cont']['max_increment'])
    ...
    ds_cont = BiDirectionalDsContinuation(problem,
                                          ...
                                          single_descent=cont_cfg['cont']['single_descent'],
                                          # max_increment=cont_cfg['cont']['max_increment'],
                                          )
    return ds_cont
...
def save_cont_resuls(results, results_folder, cfg, cont_cfg):
    filename = filename_from_cfg(cont_cfg)
    res_path = os.path.join(results_folder, 'cont', filename)
    if cfg['save_results']:
